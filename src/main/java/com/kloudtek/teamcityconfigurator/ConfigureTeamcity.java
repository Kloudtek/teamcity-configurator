...
public class ConfigureTeamcity {
    ...
    @Parameter(names = {"-st", "--setup-timeout"}, description = "How long to wait for successful server setup")
    private long setupTimeout = 2 * 60000L;
    ...
    public void execute() throws SetupException {
        ...
        long setupTimelimt = System.currentTimeMillis() + setupTimeout;
        while (!completed) {
            try {
                ...
                if (System.currentTimeMillis() > setupTimelimt) {
                    throw new SetupException(e);
                } else {
                    System.out.println("Error occured setting up, retrying: " + e.getMessage());
                    if (debug) {
                        e.printStackTrace();
                    }
                }
